...
> ### 260526 Re-correction — MSA path plurality is now uniform
>
> Per the user-supplied authoritative swagger 2.3.0 (2026-05-26), **plural `msa-contracts` is canonical for all MSA endpoints across all roles**, with one named singular helper exception: `GET /manager/msa-contract` (returns all MSAs in simplified picker shape, no pagination).
>
> This supersedes the singular-vs-plural framing used throughout this document. The 2026-05-26 working-tree global rewrite migrated all source / tests / specs from singular to plural; that rewrite is correct against the live BE. See memory entry [`msa-contracts-plural-canonical-260526`](../../../../memory).
>
> **Treat anywhere below that says "singular `msa-contract`" as historical context.** The audit-time frontend bugs documented in §3 / §5 / §6 / §10 / §11 / §12 were all fixed by either the per-wave commits listed in the table below or the global migration.

**False positives — RETRACTED, do not "fix"** *(framing reflects the 260524 spec snapshot; see 260526 re-correction above for the current model):*

- **§11 Invoice "msa-contracts (plural) typo"** ❌ **RETRACTED.** The BE genuinely accepted and routed `/manager/msa-contracts/{contractId}/invoice/{invoiceId}/approve` with plural `msa-contracts` — at the 260524 spec snapshot, this was the only MSA endpoint that used the plural form while every other MSA endpoint used singular `msa-contract`. The frontend code at `Invoice.tsx:120-122` correctly mirrored that BE shape. (Post-260526: plural is now canonical everywhere, so the original "typo" framing no longer applies regardless.)
- **§6 RFI "manager singular `/rfi` should be plural `/rfis`"** ❌ **RETRACTED.** BE genuinely used singular `/rfi` for MSA (snapshot at 260524 had MSA contract noun as `msa-contract`). The audit assumed MSA followed Contract's manager-plural-`rfis` quirk; it didn't. Frontend code was correct.
- **§12 Vendor Reports "wrong endpoint family"** ❌ **RETRACTED.** No `/msa-contracts/*/reports` route exists in swagger. MSA Vendor Reports correctly shares Contract endpoints under `/{role}/contracts/{id}/reports`. Frontend code is correct.
- **§3 Claims "Manager can't Create Claim on MSA"** ❌ **RETRACTED** (Wave 2 pre-flight). `/manager/msa-contracts/{contractId}/claims` is GET-only on BE — no POST endpoint exists. MSA's gating to vendor/PM only is BE-correct. Contract behavior (manager can POST a claim) does not exist on MSA. Frontend code is correct.
...
| §5 NCR Log basePath `/contracts/` → `/msa-contracts/` for all 4 roles | 1 | ✅ Shipped | `51df46d56` |
| §10 LEM basePath `/contracts/` → `/msa-contracts/` for all 4 roles | 1 | ✅ Shipped | `89e453fa8` |
...
| §1 §2 §3 §4 §6 §7 §8 §9 — `ExportReportSheet` wrapping on 8 MSA tabs (Amendments, Change Mgmt, Claims, Deliverables, RFI, Compliance, Documents, Payment Summary) with `contractType="MsaContract"` | 4 | ✅ Shipped | `2f8b27204` |
| §5 NCR Log body swapped to shared `NcrTable` + `NcrStatsCards` + `CreateNcrDialog` (restores CAPA workflow, Close-NCR checklist, Create button, role-aware actions, status tone — closes most §5 findings in one move) | 5 | ✅ Shipped | `6d26a28f0` |
| §2 Change Mgmt stats variant role-aware (`variant={isApprover ? "approver" : "manager"}`) — fixes manager stats hitting approver field aliases | 6 | ✅ Shipped | `8efa184f6` |

**Wave 2 closes here.** All five Wave-2 actionable findings have been addressed (§3 retracted, §4 / §8 / §10 / §12 shipped). **Wave 3 closes here.** Five of six recurring-drift entries swept; §11 Invoice excluded — `ContractInvoiceDTO` status enum has no `closed` value at the type level (`"active" | "draft" | "pending" | "approved" | "rejected"`), so the audit's recurring-pattern table over-counted Invoice. **Wave 4 closes here.** All 8 dead Export Report buttons now functional — `ExportReportSheet` already supported `contractType="MsaContract"` (component branches on it for filename, toast text, and type query param). **Wave 5 closes here.** §5 NCR Log body refactored from 486 → 76 lines by swapping to shared `NcrTable` + `NcrStatsCards` + `CreateNcrDialog`. The shared `NcrTable` already implements the full CAPA workflow per memory `project_ncr_capa_workflow` (responder submits CAPA, submittedBy approves + closes via 8-item checklist, identity match at `user._id`). Side effects: query keys un-wrapped (no more `useUserQueryKey` invalidation trap on this tab), inline `NcrDetailsSheet` stub removed, status tone via canonical `NcrTable` palette. **Wave 6+ remains open** per the original fix sequencing.
...
1. **MSA NCR Log, LEM, and Vendor's Reports all fetch from `/contracts/` instead of `/msa-contracts/`** (§5, §10, §12) — likely returns wrong data, empty, or 404 for every role. Single most impactful family of bugs.
2. **MSA Compliance manager Approve/Reject endpoint is missing the `/contract` prefix** — resolves to wrong URL (§7).
3. **MSA Invoice manager Approve/Reject "plural-with-trailing-s" typo** — already retracted in the Post-Audit Corrections block; at the audit snapshot this was the only plural endpoint while the rest of the family was singular, BE accepted both. Now obsolete per the 260526 re-correction: plural is canonical (§11).
...
1. **Wave 1 — Endpoint corrections (small diff, high impact):**
   NCR (§5), LEM (§10), Vendor Reports (§12): switch `basePath` to `/msa-contracts/`. Compliance manager Approve (§7): add `/contract` prefix. Invoice manager Approve (§11): originally framed as "fix plural-with-s back to singular" — now obsolete per 260526 re-correction; entire MSA family is plural. Claims (§3): make `/claims` plural for all roles. RFI (§6): make manager `/rfis` plural.
...
|------|---------------|----------|-------|
| Manager | `/contract/manager/contracts/{id}/amendments` (plural) | `/contract/manager/msa-contracts/{id}/amendments` (plural) | Both manager paths plural; vendor/approver/user paths singular `amendment`. Known BE quirk. |
| Vendor / PM | `/contract/vendor/contracts/{id}/amendment` (singular) | `/contract/vendor/msa-contracts/{id}/amendment` (singular) | ✅ shape matches |
| Approver | `/contract/approver/contracts/{id}/amendment` | `/contract/approver/msa-contracts/{id}/amendment` | ✅ shape matches |
| View-only | `/contract/user/contracts/{id}/amendment` | `/contract/user/msa-contracts/{id}/amendment` | ✅ shape matches |
...
|------|---------------|----------|
| Manager | `/contract/manager/contracts/{id}/changes` | `/contract/manager/msa-contracts/{id}/changes` |
| Vendor / PM | `/contract/vendor/contracts/{id}/changes` | `/contract/vendor/msa-contracts/{id}/changes` |
| Approver | `/contract/approver/contracts/{id}/changes` | `/contract/approver/msa-contracts/{id}/changes` |
| View-only | `/contract/user/contracts/{id}/changes` | `/contract/user/msa-contracts/{id}/changes` |
...
- **[Drifted] All roles — `documentType="MsaContract"` extra prop** — MSA passes `documentType="MsaContract"` to `CreateChangeDialog` (`ChangeManagement.tsx:283`); Contract does not. This is presumably wired inside `CreateChangeDialog` to route to a different `/msa-contracts/` create endpoint; verify the dialog respects it (read `CreateChangeDialog.tsx` during fix).
...
| Role | Contract base | MSA base | Notes |
|------|---------------|----------|-------|
| Manager | `/contract/manager/contracts/{id}/claims` | `/contract/manager/msa-contracts/{id}/claims` | ✅ both plural |
| Admin | (treated as ViewOnly: `…/user/…/claims`) | `/contract/manager/msa-contracts/{id}/claims` (admin treated as manager) | ⚠️ admin routes differ |
| Vendor / PM | `/contract/vendor/contracts/{id}/claims` (plural) | `/contract/vendor/msa-contracts/{id}/claim` (**singular**) | ❌ MSA still singular |
| Approver | `/contract/approver/contracts/{id}/claims` (plural) | `/contract/approver/msa-contracts/{id}/claim` (**singular**) | ❌ MSA still singular |
| View-only | `/contract/user/contracts/{id}/claims` (plural) | `/contract/user/msa-contracts/{id}/claim` (**singular**) | ❌ MSA still singular |
...
| Role | Contract base | MSA base |
|------|---------------|----------|
| Manager (+ companyAdmin/superAdmin) | `/contract/manager/contracts/{id}/deliverables` | `/contract/manager/msa-contracts/{id}/deliverables` |
| Vendor / PM | `/contract/vendor/contracts/{id}/deliverables` | `/contract/vendor/msa-contracts/{id}/deliverables` |
| Approver | `/contract/approver/contracts/{id}/deliverables` | `/contract/approver/msa-contracts/{id}/deliverables` |
| View-only | `/contract/user/contracts/{id}/deliverables` | `/contract/user/msa-contracts/{id}/deliverables` |
...
| Role | Contract base | MSA base | Notes |
|------|---------------|----------|-------|
| Manager | `/contract/manager/contracts/{id}/ncrs` | `/contract/manager/contracts/{id}/ncrs` | ❌ MSA points at **Contract** endpoint, not `/msa-contracts/` |
...
- **[Broken] All roles — Endpoint targets `/contracts/`, not `/msa-contracts/`** — Every branch in MSA's `basePath` memo points at the regular Contract endpoint (`NcrLog.tsx:261-276`). Other MSA tabs (Amendments, Change Mgmt, Claims, Deliverables, etc.) all use `/msa-contracts/{id}/…`. Either the NCR endpoint was never moved or this is a literal copy-paste oversight. Net result for every role: MSA NCR tab fetches and displays the regular Contract's NCRs (if contractId collides), or returns empty/404. **Highest-priority fix in this audit.**
...
| Role | Contract base | MSA base | Notes |
|------|---------------|----------|-------|
| Manager | `/contract/manager/contracts/{id}/rfis` (**plural**) | `/contract/manager/msa-contracts/{id}/rfi` (**singular**) | ❌ MSA may 404 if BE follows Contract's quirk |
| Vendor / PM | `/contract/vendor/contracts/{id}/rfi` | `/contract/vendor/msa-contracts/{id}/rfi` | ✅ |
| Approver | `/contract/approver/contracts/{id}/rfi` | `/contract/approver/msa-contracts/{id}/rfi` | ✅ |
| View-only | `/contract/user/contracts/{id}/rfi` | `/contract/user/msa-contracts/{id}/rfi` | ✅ |
...
- **[Broken] Manager — Endpoint singular `rfi` vs Contract's plural `rfis`** — Contract: `/manager/contracts/{id}/rfis` (`RfiTabContent.tsx:428`). MSA: `/manager/msa-contracts/{id}/rfi` (`Rfi.tsx:826`). If MSA BE follows the same singular/plural rule as Contract (manager → plural; others → singular), this 404s for manager.
...
| Role | Contract base | MSA base |
|------|---------------|----------|
| Manager | `/contract/manager/contracts/{id}/compliance` | `/contract/manager/msa-contracts/{id}/compliance` |
| Vendor / PM | `/contract/vendor/contracts/{id}/compliance` | `/contract/vendor/msa-contracts/{id}/compliance` |
| Approver | `/contract/approver/contracts/{id}/compliance` (tab hidden — early-return null) | `/contract/approver/msa-contracts/{id}/compliance` (tab visible) |
| View-only | `/contract/user/contracts/{id}/compliance` (tab hidden) | `/contract/user/msa-contracts/{id}/compliance` (tab visible) |
...
| Role | Contract | MSA |
|------|---------|-----|
| Manager | (Files come from parent contract-detail data; no role-distinct fetch.) | `/contract/manager/msa-contracts/{id}` (detail endpoint) |
| Vendor / PM | (parent) | `/contract/vendor/msa-contracts/{id}` |
| Approver | (parent) | `/contract/approver/msa-contracts/{id}` |
| View-only | (parent) | `/contract/user/msa-contracts/{id}` |
...
| Role | Contract | MSA |
|------|---------|-----|
| Manager | `contractManagerApi.listPaymentHoldbacks/Savings(contractId)` → `/contract/manager/contracts/{id}/payment-holdbacks` and `…/payment-savings` | `/contract/manager/msa-contracts/{id}/payment-holdbacks` and `…/payment-savings` |
| Vendor / PM | `/contract/vendor/contracts/{id}/payment-holdbacks` (savings intentionally skipped) | `/contract/vendor/msa-contracts/{id}/payment-holdbacks` (savings intentionally skipped) |
| Approver | `/contract/approver/contracts/{id}/payment-holdbacks` and `…/payment-savings` | `/contract/approver/msa-contracts/{id}/payment-holdbacks` and `…/payment-savings` |
...
| Role | Contract base | MSA base | Notes |
|------|---------------|----------|-------|
| Manager | `/contract/manager/contracts/{id}/lems` | `/contract/manager/contracts/{id}/lems` | ❌ MSA points at **Contract** endpoint, not `/msa-contracts/` |
...
| Role | Contract base | MSA base | Notes |
|------|---------------|----------|-------|
| Manager | `/contract/manager/contracts/{id}/invoice` | `/contract/manager/msa-contracts/{id}/invoice` | ✅ list shape; ❌ **approve path uses `msa-contracts` (plural)** |
| Vendor / PM | `/contract/vendor/contracts/{id}/invoice` | `/contract/vendor/msa-contracts/{id}/invoice` | ✅ |
| Approver | `/contract/approver/contracts/{id}/invoice` | `/contract/approver/msa-contracts/{id}/invoice` | ✅ |
| View-only | `/contract/user/contracts/{id}/invoice` | `/contract/user/msa-contracts/{id}/invoice` | ✅ |
...
| Role | Contract base | MSA base | Notes |
|------|---------------|----------|-------|
| Manager | `/contract/manager/contracts/{id}` (then `/reports`) | `/contract/manager/contracts/{id}/reports` | ❌ MSA points at **Contract** endpoint, not `/msa-contracts/` |
...
- **[Broken] All roles — Endpoint targets `/contracts/`, not `/msa-contracts/`** — `Reports.tsx:176-182`. Third tab in Group B with the same bug (alongside NCR §5 and LEM §10). All MSA reports fetches and the detail sheet hit the Contract endpoint family.
