...
async function setupRoutes(page: Page, msaId: string, invoiceId: string) {
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (
      pathname.endsWith(`/contract/manager/msa-contract/${msaId}/invoice/stats`) &&
      method === "GET"
    ) {
      await route.fulfill({
        status: 200,
    ...
    if (
      pathname.endsWith(`/contract/manager/msa-contract/${msaId}/invoice`) &&
      method === "GET"
    ) {
      await route.fulfill({
        status: 200,
    ...
    if (
      pathname.endsWith(`/contract/manager/msa-contract/${msaId}/invoice/${invoiceId}`) &&
      method === "GET"
    ) {
      await route.fulfill({
        status: 200,
