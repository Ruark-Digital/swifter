...
const ClaimDetailsSheet: React.FC<Props> = ({
  ...
  }, [basePath, isManager, isApprover, isVendor, isAdmin, isViewOnly, contractId]);
        ...
        <div data-testid="claim-details-sheet" className="flex flex-col h-full">
          <SheetHeader className="p-6 pb-4 border-b border-slate-200">
            ...
            <div className="flex items-center gap-3 mt-4 ml-11">
              <Badge variant="secondary" className="capitalize text-slate-700 bg-slate-100">
                {status}
              </Badge>
              <span className="text-sm text-slate-500">ID: {detail?.claimId ?? claimId}</span>
            </div>
          </SheetHeader>
